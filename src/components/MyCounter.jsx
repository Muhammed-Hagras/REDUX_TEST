...
export default function MyCounter() {
  ...
  const { count, showCount } = useSelector((state) => state.counterReducer);
  const { loggedIn } = useSelector((state) => state.authReducer);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatach(increase(5))
  }, [dispatach])
  

  const logHandler = () => {
    loggedIn ? dispatach(logOut()) : dispatach(logIn())
  }
  return (
    <div>
      <h2>Hello Redux </h2>
        ...
        <>
          ...
          <div className="btns">
            <button
              ...
              onClick={() => dispatach(increase(4))}
            >
              Increase +
            </button>
            <button
              className="btn btn-danger mx-3 px-3"
              onClick={() => dispatach(decrease(2))}
            >
              Decrease -
            </button>
          </div>
      ...
      <button
        className="btn btn-warning my-3 mx-auto px-3 d-block"
        onClick={()=>logHandler()}
      >
